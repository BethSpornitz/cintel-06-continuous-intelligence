...
DATA_FILE: Final[Path] = DATA_DIR / "system_metrics_bethspornitz.csv"
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "system_assessment_bethspornitz.csv"
...
def main() -> None:
    ...
    df = df.with_columns(
        [
            ...
            pl.when(pl.col("errors") > 0)
            .then(pl.col("requests") / pl.col("errors"))
            .otherwise(None)
            .alias("requests_per_error"),
        ]
    )
